<template>
  <div style="margin-bottom: 50px">
    <div v-for="(item, idx) in moduleData" :key="idx">
      <h2>{{ item.title }}</h2>
      <br />
      <h2 class="second">{{ item.subTitle }}</h2>
      <br />
      <div
        v-for="(child, index) in item.chartDataList"
        :key="index"
        class="charts-wrap"
      >
        <CommonEcharts :chart-data="child" :auto-resize="Boolean(true)" />
      </div>
...
export default {
  ...
  mounted() {
    // this.moduleEduInfo = this.$props.moduleData
  },
};
</script>
<style scoped>
